...
def _analyze_video(path: str):
    ...
    for _, frame, landmarks in key_frames:
        ...
        for name, val in angles.items():
            ...
            y_offset += 30
        _, buffer = cv2.imencode(".jpg", skeleton_frame, [cv2.IMWRITE_JPEG_QUALITY, 80])
        import base64
        annotated_b64.append(base64.b64encode(buffer).decode("utf-8"))
...
async def analyze_video(file: UploadFile = File(...)):
    try:
        ...
        try:
            result, _ = _analyze_video(temp_path)
        finally:
